...
parser.add_argument('--png_name', type=str, default='Caly the Cattrap', help='Name of the PNGtuber')
args = parser.parse_args()

# Ruta de las imágenes y el audio
default_pngtuber_name = "Caly the Cattrap"
base_dir = "PNGtubers"
pngtuber_name = args.png_name
...
emotions_similar = {
    ...
    "Llorando": ["Llorando", "Sollozando", "Lagrimando", "Desconsolada", "Desconsolado"],
...
emotions = ["Aburrida", "Emocionada", "Enojada", "Llorando", "Nerviosa", "Neutral", "Pensativa", "Riendo", "Sorprendida"]
...
def pngtuber():
    ...
    while running:
        ...
        vertical_position = screen_height // 2 + int(amplitude_sine * np.sin(2 * np.pi * frequency_sine * timey))
...
def _new_loop(gpt4all_instance):
    ...
    prev_message = ""                   
    message_counter=0
    with gpt4all_instance.chat_session(system_prompt=initial_message):
        salida_texto=""
        while True:
            ...
            if message_counter >= 10:
                initial_message = read_initial_message()  # Leer el mensaje inicial
                message = initial_message + "\n" + message  # Combinar con el mensaje actual
                message_counter = 0  # Reiniciar el contador
            ...
            for token in response_text:
                ...
                if token.endswith("\n") or token_futuro.endswith("["):
                    ...
                    if cadena_valida(texto) and initial2==True:
                        tmp_emotion+=emotion
